...
(function(){
	...
	var SwipeView = (function (window, document) {
		var dummyStyle = document.createElement('div').style,
			...
			SwipeView = function (el, options) {
				...
				div = this.wrapper.querySelector(".swipeview-sbox");
				...
				this.refreshSize();
				var __divs = div.querySelectorAll(".swipeview-sbox>div");
				this.options.numberOfPages = __divs.length;
				this.__cacheFrag = document.createDocumentFragment();

				for (var i = 0; i < __divs.length; i++) {
					this.__cacheFrag.appendChild(__divs[i]);
				};
				for (i=-1; i<2; i++) {
					div = document.createElement('div');
					// div = i<0 ? __divs[__divs.length+i] : __divs[i];
	...
	function moveChilds (source,target,gallery) {
		var cs = source.childNodes;

		if (cs.length == 0 && source._SPViewIndex >= 0) {
			moveChilds(gallery.masterPages[source._SPViewIndex],source);
		};
		
		var xx = document.createDocumentFragment();
		while (source.childNodes.length > 0){
			xx.appendChild(source.childNodes[0]);
		}
		
		target.appendChild(xx);
		xx = null;
	}

	$.fn.extend({
		swipeview : function (opt) {
			
			var gallery = new SwipeView(this[0], opt);
			var dots = this.find(".swipeview-nbox>div");
			// Load initial data
			for (i=0; i<3; i++) {
				var page = i==0 ? gallery.__cacheFrag.childNodes.length-1 : i-1;
				moveChilds(gallery.__cacheFrag.childNodes[page],gallery.masterPages[i],gallery);
				gallery.masterPages[i]._SPDataIndex = page;
				gallery.__cacheFrag.childNodes[page]._SPViewIndex = i;
			}
			...
			gallery.onFlip(function () {
				...
				for (i=0; i<3; i++) {
					...
					if (upcoming != gallery.masterPages[i].dataset.pageIndex) {
						...
						moveChilds(gallery.masterPages[i], gallery.__cacheFrag.childNodes[gallery.masterPages[i]._SPDataIndex],gallery);
						moveChilds(gallery.__cacheFrag.childNodes[upcoming],gallery.masterPages[i],gallery);
						gallery.masterPages[i]._SPDataIndex = upcoming;
						gallery.__cacheFrag.childNodes[upcoming]._SPViewIndex = i;

						_this.find(".swipeview-nbox>div.active").removeClass('active');

						dots.eq(gallery.pageIndex).addClass('active');
					}
				}
