...
(function($){
	...
	function clearAllHandle (ele, type, capture) {
		if (fnArr[_id(ele)] && fnArr[_id(ele)][type]) {
			for (var i = 0; i < fnArr[_id(ele)][type].length; i++) {
				capture === fnArr[_id(ele)][type][i].capture && (fnArr[_id(ele)][type] = fnArr[_id(ele)][type].splice(i,1))
			}
		};
	}
	...
	function _removeEvent (ele, types, fn, capture) {
		if (typeof types !== "string" || !ele._iTEID) {
			return;
		};
		...
		//删除所有绑定的事件
			...
			eachTypes(types, function (i, type) {
				...
				//清空对应的handles
				clearAllHandle(ele, type, fn , capture);
			});
		}
	...
	$.Event = function(type, props) {
		...
		var event = document.createEvent(MouseEvs.indexOf(type) !== -1 ? "MouseEvents" : 'Events'), bubbles = true
