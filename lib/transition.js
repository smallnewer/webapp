...
(function($){
	...
	function _startAni (propObj, opt) {
		...
		function warpHandle (ev) {
			...
			_fire(ev.propertyName);

		}
		var fireNum = 0;

		var _fire = $.proxy(function (name) {
			//触发以后就删除，避免重复触发
			var $ele = this;
		...
		$ele[0].clientLeft;
		var nowProps = addProp($ele, propObj, opt.time, opt.ease);
	...
	function addProp ($ele, propObj ,time, ease) {
		...
		for (var i = 0; i < nowProps.length; i++) {
			res += "," + nowProps[i] + " " + nowTimes[i] + " " + nowEases[i];
		};
	...
	function startAni (propObj, opt) {
		return _startAni.apply(this,arguments)
		return this;
	}
