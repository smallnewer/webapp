(function(win,doc){
	...
	$.fn = $.prototype = {
		...
		// let $ object like a array;
		length: 0,
		push: [].push,
		sort: [].sort,
		splice: [].splice // maybe only need splice
	}
	...
	$.extend = $.fn.extend = function () {
		...
		for (var key in clone){
			if (!(key in target)) {
				//clone
				if (deep && ($.isObj(clone[key]) || $.isArr(clone[key]))) {
					target[key] = $.isObj(clone[key]) ? {} : [];
					target[key] = $.extend(target[key], clone[key], deep);
				}else{
	...
	$.fn.extend({
		...
		css : function (key, value) {
			...
			if (arguments.length == 1 && this[0]) {
				if ($.isObj(key)) {
					for (var i in key) {
						i = ($.isFn($.getSupportStyle)) ? $.getSupportStyle(i) : i;
						this.css(i, key[i]);
					};
					return this;
				}else{
					key = ($.isFn($.getSupportStyle)) ? $.getSupportStyle(key) : key;
			...
			switch (arguments.length){
				...
				case 1:
					if (!_dataCache[flag]) {
						...
						var x = this.attr("data-" + $.toCSSCase(key));
				...
				case 2:
					return this.each(function (i,item) {
						if (!item._dataFlag) {
							item._dataFlag = "iTools" + Date.now();
						};
						var flag = item._dataFlag;
						if (!_dataCache[flag]) {
							_dataCache[flag] = {};
						};
